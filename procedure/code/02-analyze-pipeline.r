...
pipelineTweetsByHour <- ts_data(pipeline, by="hours")
ts_plot(pipeline, by="hours")
...
pipelineNetwork <- network_graph(pipeline, c("quote"))

plot.igraph(pipelineNetwork)
...
pipelineText = pipeline %>% select(text) %>% plain_tweets()

# parse out words from tweet text
pipelineWords = pipelineText %>% unnest_tokens(word, text)
...
stop_words = stop_words %>% 
  ...
  add_row(word="colonial",lexicon = "Search") %>%
  add_row(word="pipeline",lexicon = "Search") %>%
  add_row(word="cyberattack",lexicon = "Search") %>% 
  add_row(word="cybersecurity",lexicon = "Search") 
  

#delete stop words from dorianWords with an anti_join
pipelineWords =  pipelineWords %>% anti_join(stop_words) 
...
pipelineWordPairs = pipelineText %>% 
...
ggplot() +
  ...
  geom_point(data = pipeline, aes(x=lng,y=lat),
             colour = 'purple', alpha = .2) +
  labs(title = " Tweets During the Colonial Pipeline Incident")+
