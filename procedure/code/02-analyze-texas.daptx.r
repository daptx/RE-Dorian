...
texasTweetsByHour <- ts_data(texas, by="hours")
ts_plot(texas, by="hours")
...
texasNetwork <- network_graph(texas, c("quote"))

plot.igraph(texasNetwork)
...
texasText = texas %>% select(text) %>% plain_tweets()

# parse out words from tweet text
texasWords = texasText %>% unnest_tokens(word, text)
...
stop_words = stop_words %>% 
  ...
  add_row(word="appointment",lexicon = "Search") %>% 
  add_row(word="vaccination",lexicon = "Search")

#delete stop words from dorianWords with an anti_join
texasWords =  texasWords %>% anti_join(stop_words) 
